...
bool BurgerOldVersion::canMoveSideway() {
    return !(x > STOP_BURGER_X_LEFT && x < STOP_BURGER_X_RIGHT);
}

int BurgerOldVersion::xDirectionSign() {
...
void BurgerOldVersion::move(double deltaTicks) {
    if(canMoveSideway()) {
...
bool BurgerOldVersion::canMoveDown() {
...
bool BurgerOldVersion::isGameOver() {
    return !canMoveDown();
}
